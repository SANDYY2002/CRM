...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Conversation.objects.filter(organization=org).select_related(
    ...
    def mark_read(self, request, pk=None):
        ...
        conversation.messages.filter(
            is_read=False, direction=Message.Direction.INBOUND
        ).update(is_read=True)
...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        conversation = get_object_or_404(Conversation, pk=conversation_id, organization=org)
