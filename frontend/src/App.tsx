...
function Dashboard() {
  const { user, organizations, clearSession } = useAuthStore();
  ...
  return <main className="app-shell">
    ...
    <aside className="sidebar glass-panel">
      ...
      <div className="workspace"><div className="workspace-avatar">{organization?.name?.[0] || 'C'}</div><div><strong>{organization?.name || 'CRM Workspace'}</strong><small>Business workspace</small></div><ChevronDown size={15} /></div>
      <nav>
    ...
    <section className="content">
      <header className="topbar">
        ...
        <div className="top-actions"><button className="icon-button"><Bell size={19} /><i /></button><div className="profile"><div className="avatar">{user?.first_name?.[0] || 'C'}{user?.last_name?.[0] || ''}</div><div><strong>{user?.first_name || user?.username || 'User'}</strong><small>{user?.role || 'Agent'}</small></div></div><button className="icon-button" title="Sign out" onClick={clearSession}><LogOut size={17} /></button></div>
      </header>
      <div className="page-heading"><div><p className="eyebrow">CRM WORKSPACE</p><h1>Good evening, {user?.first_name || user?.username || 'there'} <span>👋</span></h1><p className="muted">Here's what's happening across your customer channels today.</p></div><button className="primary-button">+ New customer</button></div>
      <div className="stats-grid">{stats.map(({ label, value, change, icon: Icon }, index) => <motion.div key={label} className="stat-card glass-card" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .06 }}><div className="stat-top"><div className="stat-icon"><Icon size={19} /></div><span className="trend">{change}</span></div><strong>{value}</strong><span>{label}</span></motion.div>)}</div>
...
function App() {
  ...
  const { user, hydrated, setSession, clearSession } = useAuthStore();
  useEffect(() => {
    useAuthStore.getState().hydrate();
    if (!hasAccessToken()) { setChecking(false); return; }
    api.me().then((result) => setSession(result.user, result.organizations, localStorage.getItem('crm_access_token') || '', localStorage.getItem('crm_refresh_token') || '')).catch(() => clearSession()).finally(() => setChecking(false));
  }, [setSession, clearSession]);
  if (checking || !hydrated) return <div className="loading-screen"><div className="loading-orb" /><span>Preparing your workspace…</span></div>;
